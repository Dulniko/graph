    ...
    def is_valid_assignment(self, u: int, match: List[int], seen: List[bool]) -> bool:
        for v in range(self.num_front, self.V):
            if self.graph[u][v] and not seen[v]:
                seen[v] = True
                if match[v - self.num_front] == -1 or self.is_valid_assignment(match[v - self.num_front], match, seen):
...
def main():
    ...
    print("Podaj pary tragarzy, którzy mogą ze sobą współpracować (format: przod tyl, np. 0 1):")
